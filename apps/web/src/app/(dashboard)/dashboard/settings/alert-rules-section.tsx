...
import type { AlertRule, ApiSource, AlertChannel, Severity } from '@/lib/types';
import { SEVERITY_ORDER, getTeamId } from '@/lib/shared';
import { useDemo } from '@/lib/use-demo';
import { DEMO_ALERT_RULES, DEMO_SOURCES } from '@/lib/demo-data';
...
export function AlertRulesSection() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [rulesData, sourcesData] = await Promise.all([
      ...
      toast.error(e instanceof Error ? e.message : 'Could not load alert rules. Please try again.');
    } finally {
      setLoading(false);
    }
  }, [teamId, isDemo]);
  ...
  return (
    ...
    >
      <div className="mb-5 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Configure where APIDelta sends notifications when API changes are detected. Delivery status appears inline on the Changes feed.
          </p>
        </div>
        <button
          onClick={() => setShowCreateForm(true)}
          disabled={loading}
          className="inline-flex shrink-0 items-center gap-2 rounded-lg bg-violet-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-violet-500 disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 focus-visible:ring-offset-2 focus-visible:ring-offset-gray-950"
        >
          ...
          <div className="w-full max-w-lg rounded-xl border border-gray-800 bg-gray-950 p-6 shadow-2xl animate-modal-content max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleCreateRule} className="space-y-5" noValidate>
              ...
              <div className="rounded-xl border border-gray-800 bg-gray-900/30 p-5 space-y-4">
                ...
                <input
                  id="rule-destination"
                  type={formChannel === 'EMAIL' ? 'email' : formChannel === 'GITHUB' ? 'text' : 'url'}
                  inputMode={formChannel === 'EMAIL' ? 'email' : formChannel === 'GITHUB' ? 'text' : 'url'}
      ...
      {loading && rules.length === 0 ? (
        ...
        <div className="rounded-xl border border-dashed border-gray-800 px-6 py-12 text-center">
          ...
          <p className="mt-1 max-w-sm mx-auto text-xs text-gray-600">
            {sources.length === 0
              ...
              : 'Create a rule to start receiving Slack, email, webhook, or GitHub notifications when API changes are detected.'}
          </p>
          {sources.length === 0 ? (
            <Link
        ...
        <div className="space-y-3">
          {rules.map((rule) => {
            const source = sources.find((s) => s.id === rule.sourceFilter);
            const ChannelIcon =
              rule.channel === 'EMAIL'
                ? Mail
                : rule.channel === 'SLACK'
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`inline-flex items-center gap-1 text-xs ${
                          rule.isActive ? 'text-emerald-400' : 'text-gray-600'
                        }`}
                      >
                        <span
                    ...
                    <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-gray-500">
                      ...
                      <span
                        className={`rounded px-1.5 py-0.5 text-[10px] font-medium ${
                          SEVERITY_BADGE[rule.minSeverity] ?? 'bg-gray-800 text-gray-400'
                        }`}
                      >
                        {rule.minSeverity}+
                      </span>
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-1 truncate font-mono text-xs text-gray-300">
                          {isRevealed ? rule.webhookSecret : '•'.repeat(48)}
                        </p>
                      </div>
                      <div className="flex shrink-0 items-center gap-1">
                        <button
